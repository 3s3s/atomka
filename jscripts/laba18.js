...
{
	if (element1) element1.remove().draggable(false);
	if (element2) element2.remove().draggable(false);
	...
	if (element1) group.add(element1);
	if (element2) group.add(element2);
...
utils.makeGroup = function(layer, element, callback)
{
	element.getKinetic().remove().draggable(false);
	...
	group.add(element.getKinetic());
	
	group.pins = function(){
			return {
				up: element.pins().up, 
				down: element.pins().down};
	};
...
utils.JoinUpDown = function(layer, element1, element2, callback)
{
	...
	var line = utils.JoinPinsFine(element1.pins().up, element2.pins().down);
...
utils.JoinDownUp = function(layer, element1, element2, callback)
{
	if (!element1.pins) return;
	if (!element2.pins) return;
	
	var line = utils.JoinPinsFine(element1.pins().down, element2.pins().up);
...
utils.JoinReverce = function(layer, element1, element2, callback, newUp, newDown)
{
	if (!element1.pins) return;
	if (!element2.pins) return;
	
	var lineUp = utils.JoinPinsFine(element1.pins().up, element2.pins().down);
	var lineDown = utils.JoinPinsFine(element1.pins().down, element2.pins().up);
	...
	if (lineUp && lineDown)
	{
		group.pins = function(){
			var ret = {
				up: newUp==undefined ? element2.pins().up : newUp,  
				down: newDown==undefined ? element2.pins().down : newDown};
			return ret;
		};
		group.getKinetic = function() {return group;};
	}
...
var atomka = function(containerID) {
	...
	this.addDiod = function(coords, value, callback)
	{
		this.diods.push(new atomka.Diod(coords, value, callback));
		return this.diods.length-1;				
	};
...
atomka.Diod = function(coords, callback)
{
	this.coords = [0, 0];
	...
	var line = new Kinetic.Line({
		points: [obj.x()-triangle.radius(), obj.y()-triangle.radius()-1, 
		         obj.x()+triangle.radius(), obj.y()-triangle.radius()-1],
		stroke: 'black'});

	this.pins = function()
	{
		return {
			up: [obj.x(), obj.y()-triangle.radius()-10], 
			down: [obj.x(), obj.y()+triangle.radius()/2+10]};
	};

	var wireUp = new Kinetic.Line({
		points: [obj.x(), obj.y()-triangle.radius()-1, 
		         obj.x(), obj.y()-triangle.radius()-10],
		stroke: 'black'});
	var wireDown = new Kinetic.Line({
		points: [obj.x(), obj.y()+triangle.radius()/2, 
		         obj.x(), obj.y()+triangle.radius()/2+10],
		stroke: 'black'});
	...
	obj.add(triangle, line, wireUp, wireDown);	
...
atomka.Lamp = function(coords, temperature, callback) {
	...
	this.R = (l*utils.constants.q)/(Math.PI*Math.pow(radius, 2)); //Электрическое сопротивление
...
atomka.box = function(width, height, label, units)
{
	...
	{
		textValue.text((value+"").substring(0,9));
...
window.onload = function(e){ 
	...
	laba11.onVResistorChanged = function(vResistorValue)
	{
		...
		var E = utils.constants.E;  //Коэффициент излучения нагретого тела
		var sigma = 5.6704E-8;
		
		var T = Math.pow(P/(S*E*sigma), 1/4);
		if (T > utils.constants.Tmax)
		{
			laba11.vResistors[0].setValue(parseInt(vResistorValue)+1);
			return;		
		}
	...
	laba11.onV1ResistorChanged = function(vResistorValue)
	{
		var Urev = laba11.batteries[1].getValue() * (laba11.vResistors[2].getValue() / (parseFloat(laba11.vResistors[2].getValue())+parseFloat(vResistorValue)));
		var Uph = laba11.getPhotoElementVoltage();		
		
		//laba11.voltMeters[0].Update(0, Urev - Uph);
		laba11.voltMeters[0].Update(0, Urev);

		laba11.redraw();
		
	};
	laba11.onV2ResistorChanged = function(vResistorValue)
	{		
		if (parseFloat(vResistorValue) < 1)
		{
			laba11.vResistors[2].setValue(1);
			return;
		}
			
		var Urev = laba11.batteries[1].getValue() * (vResistorValue / (parseFloat(laba11.vResistors[1].getValue())+parseFloat(vResistorValue)));		
		var Uph = laba11.getPhotoElementVoltage();		
	...
	laba11.getPhotoElementVoltage = function()
	{
	...
	laba11.onPhotoElementMoved = function()
	{
		...
		var Uph = laba11.getPhotoElementVoltage();//0;
		var Urev = 0;
		var R = 1;
		if (laba11.batteries.length > 1 && laba11.vResistors.length > 2)
		{
			R = parseFloat(laba11.vResistors[2].getValue());
			Urev = laba11.batteries[1].getValue() * (R / (R+parseFloat(laba11.vResistors[1].getValue())));
		}
		
		if (Uph < Urev) Uph = Urev;
		...
		var scale = 0.000001;
		var amperageResult = (scale*(Uph - Urev)*spectr[500])/(distance * (R+utils.constants.Rph));
		...
		//laba11.ShowConstant("Urev", "Обратное напряжение<br>Uобр = " + (Urev+"").substr(0,9), "V");
	...
	laba11.onLampMoved = function()
	{
		this.y(0);
		if (this.x() < 110) this.x(110);
		if (this.x() > 200) this.x(200);
	};
	laba11.onRightGroupMoved = function()
	{
		this.x(140);
		this.y(0);
	};

	laba11.addBattery([-100, 110], utils.constants.U);
	...
	laba11.addPhotoElement([600, 100], laba11.onPhotoElementMoved);
	laba11.addAmperMeter([700, 100], 0);
	laba11.addMonoHromator([440, 90], 200, laba11.onMonohromatorChanged);
	...
	var dX = 178;
	var dY = 70;
	laba11.addBattery([90+dX, 350+dY], 3);
	laba11.addVResistor();
	laba11.addVResistor();
	...
	var group12 = utils.Join(laba11.layer, laba11.batteries[0], group11, laba11.onLampMoved);

//	var groupPhotoElement = utils.makeGroup(laba11.layer, laba11.photoElements[0]);
//	groupPhotoElement.x(50);
	...
	window.setInterval(function() {laba11.onV1ResistorChanged(laba11.vResistors[1].getValue())}, 1000);
	window.setInterval(function() {laba11.onV2ResistorChanged(laba11.vResistors[2].getValue())}, 1000);
	//laba11.onPhotoElementMoved();
