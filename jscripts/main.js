...
utils.constants = {
		...
		radius: 0.000015,	//Радиус нагретого тела в метрах
...
utils.JoinDownDown = function(layer, element1, element2, callback)
{
	if (!element1.pins) return;
	if (!element2.pins) return;
	
	var line = utils.JoinPinsFine(element1.pins().down, element2.pins().down);
...
window.onload = function(e){ 
	...
	laba11.onVResistorChanged = function(vResistorValue)
	{
		...
		var U = laba11.batteries[0].getValue()*(parseFloat(laba11.lamps[0].R)/R);
		var I = U/R;
		var P = I*U;
	...
	laba11.onPhotoElementMoved = function()
	{
		...
		laba11.voltMeters[0].Update(0, U);
		laba11.redraw();
	...
	laba11.addBattery([-110, 250], utils.constants.U);
	...
	laba11.lamps[0].Update(0, [350, 140], laba11.onPhotoElementMoved);
	
	laba11.setVResistor(0, [-20, 90], 1, laba11.onVResistorChanged);
	...
	var groupLamp_Ampermeter = utils.JoinUpDown(laba11.layer, laba11.amperMeters[1], laba11.lamps[0]);
	
	var groupVoltmeter = utils.Join(laba11.layer, groupLamp_Ampermeter, laba11.voltMeters[0]);
	
	var groupVResistor = utils.JoinUpDown(laba11.layer, groupVoltmeter, laba11.vResistors[0]);
	
	var groupBatary = utils.Join(laba11.layer, groupVResistor, laba11.batteries[0]);
	//utils.AddPin(groupLamp_Voltmeter, [laba11.voltMeters[0].pins().up[0], laba11.lamps[0].pins().down[1]])
	
	//var group11 = utils.JoinUpDown(laba11.layer, groupLamp_Voltmeter, laba11.vResistors[0]);
	...
	var group2 = utils.setGroup(laba11.layer, group1, groupBatary);
	var group3 = utils.setGroup(laba11.layer, group2, laba11.monoHromators[0].getKinetic());
	group3.x(120);
